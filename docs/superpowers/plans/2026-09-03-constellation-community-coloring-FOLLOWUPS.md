...
**Confirmed on the deployed page 2026-09-03:** the top-3 communities measured 46 / 38 / 29 nodes = **113 of 268 (42%)** and render `#22d3ee` / `#a78bfa` / `#f59e0b` in *both* modes. Toggling `community ⇄ layer` visibly changes only the smaller communities + the 105 isolated nodes (grey → layer colour); for ~42% of the map nothing moves. Recommend a `design-brain` pass on `PALETTE[0..2]`. Compare `snapshots/2026-09-03-constellation-community-coloring.png` (community) with `snapshots/2026-09-03-constellation-layer-mode.png` (layer).
