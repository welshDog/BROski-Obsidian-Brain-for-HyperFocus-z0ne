...
scripts/Install-GitHooks.ps1    — Installs pre-commit hooks: block .env commits + block newly-added lines that run the raw `db push` migration command (use apply_migration instead)
...
**Skill routing (Phase 6, 2026-06-11):** `GET /route?query=<task>&limit=N` — deterministic (no LLM) skill router: bidirectional word-stem match across ALL node layers seeds a 2-hop expansion, returns ranked HYPER-SILLs + supporting notes/code. Consumer: V2.4 `crew-orchestrator` injects routed skills into every `/execute` agent prompt (`GRAPH_ROUTE_URL` env, fail-open; preview via orchestrator `GET /route/preview?q=`).
